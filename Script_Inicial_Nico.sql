...
(
		...
		DETALLE_BONO VARCHAR(50) DEFAULT 'CONSULTA'
...
BEGIN
	...
	DECLARE @MIGRA_BONOS TABLE
	(
		SINTOMAS VARCHAR (255),
		NRO_DOCUMENTO_PROFESIONAL NUMERIC(18,0),
...
BEGIN
	...
	(ID_AFILIADO,ID_PROFESIONAL,FECHA_TURNO,NRO_TURNO,FECHA_COMPRA,FECHA_IMPRESION,SINTOMA,ENFERMEDAD,BONO_NUMERO)
	SELECT (SELECT ID_PERSONA FROM FLOPANICMA.PERSONA AS PERTAB WHERE PERTAB.NRO_DOCUMENTO=GD_ESQUEMA.MAESTRA.PACIENTE_DNI),
			...
			TURNO_FECHA,TURNO_NUMERO,COMPRA_BONO_FECHA,BONO_CONSULTA_FECHA_IMPRESION,
			CONSULTA_SINTOMAS,CONSULTA_ENFERMEDADES,BONO_CONSULTA_NUMERO 
	FROM GD_ESQUEMA.MAESTRA;
	
	INSERT INTO FLOPANICMA.PEDIDO_TURNO
	(ID_PEDIDO,FECHA,ID_PROFESIONAL,ID_AFILIADO)
	SELECT NRO_TURNO,FECHA_TURNO,ID_PROFESIONAL,ID_AFILIADO
	FROM @MIGRA_CONSULTAS
	WHERE (SINTOMA IS NULL AND FECHA_COMPRA IS NULL);
	
	INSERT INTO FLOPANICMA.CONSULTA
	(ID_PROFESIONAL,ID_AFILIADO,FECHA,SINTOMA,DIAGNOSTICO)
	SELECT ID_PROFESIONAL,ID_AFILIADO,FECHA_TURNO,SINTOMA,ENFERMEDAD
	FROM @MIGRA_CONSULTAS 
	WHERE (FECHA_COMPRA IS NULL AND FECHA_IMPRESION IS NOT NULL AND BONO_NUMERO IS NOT NULL);
	
	INSERT INTO FLOPANICMA.BONO
	(ID_AFILIADO,PLAN_MEDICO,NUMERO_AFILIADO,BONO_NRO,FECHA_COMPRA,FECHA_IMPRESION)
	SELECT AFI.ID_AFILIADO,AFI.PLAN_MEDICO,AFI.NRO_AFILIADO,MIGCON.BONO_NUMERO,MIGCON.FECHA_COMPRA,MIGCON.FECHA_IMPRESION
...
/* PERMITE CREAR UN NUEVO ROL DE USUARIO A PARTIR DE UNA DESCRIPCION INGRESADA POR EL USUARIO */
...
BEGIN
	--DECLARACIONES--
	DECLARE @NOMBRE_ROL VARCHAR(255) = @DESC_ROL; -- variable a usar en el mensaje de salida para el usuario
	...
	SET @DESC_ROL = FLOPANICMA.TRIM(UPPER(@DESC_ROL));
	
	--PROCEDIMIENTO--
	IF((SELECT COUNT(1) FROM FLOPANICMA.ROL AS ROL WHERE ROL.DESCRIPCION = @DESC_ROL) > 0) -- valida que el rol no se encuentre cargado en la base
	BEGIN
		SET @FLAG_ERROR = 1;
	...
	BEGIN
		...
		SET @MENSAJE = 'El rol "' + @NOMBRE_ROL + '" se ha guardado correctamente';
	END
END
GO
...
BEGIN
	...
	DECLARE @NOMBRE_ROL VARCHAR(255) = @DESC_ROL; -- variables para mostrar los cambios en el mensaje de salida
	DECLARE @NOMBRE_FUNCIONALIDAD VARCHAR(255) = @DESC_FUNCIONALIDAD;
	
	--VALIDACIONES--
	SELECT @ID_ROL = ROL.ID_ROL -- busca el rol por su descripcion en la base
	FROM FLOPANICMA.ROL AS ROL 
	WHERE ROL.DESCRIPCION = @DESC_ROL;
	...
	WHERE FUNCIONALIDAD.DESCRIPCION = @DESC_FUNCIONALIDAD;
	...
	SET @DESC_ROL = FLOPANICMA.TRIM(UPPER(@DESC_ROL)); -- elimina espacios en blanco a izquierda y derecha, luego convierte la cadena a mayusculas
	SET @DESC_FUNCIONALIDAD = FLOPANICMA.TRIM(UPPER(@DESC_FUNCIONALIDAD));
	...
	BEGIN
		...
		SET @FLAG_ERROR = 0;
		SET @MENSAJE = 'La funcionalidad "' + @NOMBRE_FUNCIONALIDAD + '" ha sido asignada correctamente al rol "' + @NOMBRE_ROL + '".';
	END
	ELSE
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'La funcionalidad "' + @NOMBRE_FUNCIONALIDAD + '" ya ha sido anteriormente asignada al rol "' + @NOMBRE_ROL + '".';
		RETURN
	END
END
GO
...
/* PERMITE MODIFICAR UN ROL EXISTENTE A PARTIR DE SU ID
   (puedo cambiar el nombre y agregar o eliminar una funcionalidad)
CREATE PROCEDURE FLOPANICMA.SP_ABM_ROL_MODIFICAR
@ID_ROL INT,
@DESC_ROL VARCHAR(255),
@ACTIVO BIT,
@FLAG_ERROR BIT = 0 OUTPUT,
...
AS
BEGIN
	IF (@DESC_ROL IS NOT NULL) AND (@DESC_ROL <> '') -- evito que guarde en la base cuando no se ingresa nada
	BEGIN
		SET @DESC_ROL = FLOPANICMA.TRIM(UPPER(@DESC_ROL));
		
		UPDATE FLOPANICMA.ROL SET DESCRIPCION = @DESC_ROL WHERE ID_ROL = @ID_ROL;
	END
