...
AS
BEGIN	
	IF(@DESC_ROL IS NULL OR @DESC_ROL = '')
	BEGIN
		SET @FLAG_ERROR = 1;
	...
	DECLARE @NOMBRE_ROL VARCHAR(255) = @DESC_ROL;
	
	SET @DESC_ROL = FLOPANICMA.TRIM(UPPER(@DESC_ROL));
	
	IF((SELECT COUNT(1) FROM FLOPANICMA.ROL AS ROL WHERE ROL.DESCRIPCION = @DESC_ROL) > 0)
	BEGIN
		SET @FLAG_ERROR = 1;
...
/* PERMITE ASIGNAR UNA FUNCIONALIDAD DEFINIDA A UN ROL EXISTENTE */

CREATE PROCEDURE FLOPANICMA.SP_ABM_ROL_ASIGNAR_FUNCIONALIDAD
@DESC_ROL VARCHAR(255),
...
BEGIN
	...
	SELECT @ID_FUNCIONALIDAD = FUNCIONALIDAD.ID_FUNCIONALIDAD
	...
	IF NOT EXISTS (SELECT 1 FROM FLOPANICMA.ROL_FUNCIONALIDAD AS ROL_FUNCIONALIDAD 
