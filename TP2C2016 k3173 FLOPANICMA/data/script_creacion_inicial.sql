...
CREATE SEQUENCE FLOPANICMA.AFILIADO_SEQ START WITH 101 INCREMENT BY 100;
...
CREATE SEQUENCE FLOPANICMA.ID_TURNO_SEQ AS INT START WITH 202165 INCREMENT BY 1;
...
CREATE UNIQUE INDEX NRO_AFILIADO_IND1 ON FLOPANICMA.AFILIADO(NRO_AFILIADO);
...
CREATE FUNCTION FLOPANICMA.PERTENECE_A_GRUPO(@NRO_AFILIADO INT)
RETURNS INT
AS
BEGIN
	DECLARE @CANT_MIEMBROS_GRUPO_FAMILIAR INT;

	SET @CANT_MIEMBROS_GRUPO_FAMILIAR = ( SELECT COUNT(*)
										  FROM FLOPANICMA.AFILIADO
										  WHERE LEFT(CONVERT(varchar(10), NRO_AFILIADO),LEN(CONVERT(varchar(10), NRO_AFILIADO))-1) =
												LEFT(CONVERT(varchar(10), @NRO_AFILIADO),LEN(CONVERT(varchar(10), @NRO_AFILIADO))-1) );
	
	RETURN @CANT_MIEMBROS_GRUPO_FAMILIAR;
END
GO

CREATE FUNCTION FLOPANICMA.SON_FAMILIARES(@AFI1 NUMERIC(18,0),@AFI2 NUMERIC(18,0))

RETURNS BIT

BEGIN

	DECLARE @NRO_AFILIADO1 INT;
	DECLARE @NRO_AFILIADO2 INT;

	SET @NRO_AFILIADO1 = (SELECT NRO_AFILIADO FROM FLOPANICMA.AFILIADO AS AFI JOIN 
						  FLOPANICMA.PERSONA AS PER ON AFI.ID_AFILIADO = PER.ID_PERSONA
						  WHERE NRO_DOCUMENTO = @AFI1);

	SET @NRO_AFILIADO2 = (SELECT NRO_AFILIADO FROM FLOPANICMA.AFILIADO AS AFI JOIN 
						  FLOPANICMA.PERSONA AS PER ON AFI.ID_AFILIADO = PER.ID_PERSONA
						  WHERE NRO_DOCUMENTO = @AFI2);

	IF (((@NRO_AFILIADO1/100)-(@NRO_AFILIADO2/100))=0)
	BEGIN
		RETURN 1;
	END
...
CREATE FUNCTION FLOPANICMA.TURNO_OCUPADO(@FECHA DATETIME,@NRO_AFILIADO INT)
...
BEGIN
	...
	SET @ID_AFILIADO =(SELECT ID_AFILIADO FROM FLOPANICMA.AFILIADO
						WHERE NRO_AFILIADO=@NRO_AFILIADO );
						
	SET @CANT_TURNOS= (SELECT COUNT(1) FROM FLOPANICMA.PEDIDO_TURNO
					  WHERE FECHA = @FECHA AND ID_AFILIADO = @ID_AFILIADO AND ID_TURNO
					  NOT IN (SELECT ID_TURNO_CANCELADO FROM FLOPANICMA.CANCELACION));
...
BEGIN
	...
	('REGISTRAR LLEGADA'),
	('REGISTRAR RESULTADO'),
	('HISTORIAL CAMBIOS PLANES MEDICOS');
	...
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM FLOPANICMA.FUNCIONALIDAD WHERE DESCRIPCION ='REGISTRAR RESULTADO')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM FLOPANICMA.FUNCIONALIDAD WHERE DESCRIPCION ='LISTADOS ESTADISTICOS')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM FLOPANICMA.FUNCIONALIDAD WHERE DESCRIPCION ='HISTORIAL CAMBIOS PLANES MEDICOS'));
	...
	VALUES (NEXT VALUE FOR FLOPANICMA.PERSONA_SEQ,'Nicolas',32947431);
	
	INSERT INTO FLOPANICMA.USUARIO (ID_PERSONA,USERNAME,PASSWORD)
	VALUES ((SELECT ID_PERSONA FROM FLOPANICMA.PERSONA WHERE NOMBRE='Nicolas'),'administrativo1',FLOPANICMA.PASSWORD_HASH('w23e'));
...
BEGIN
	...
	SELECT (NEXT VALUE FOR FLOPANICMA.AFILIADO_SEQ), ID_PLAN,
	...
	SELECT ID_AFILIADO,'AFIL' + CONVERT(VARCHAR(10),ID_AFILIADO),FLOPANICMA.PASSWORD_HASH('w23e') 
	FROM FLOPANICMA.AFILIADO;
		
	INSERT INTO FLOPANICMA.USUARIO_ROL
	(ID_USUARIO,ID_ROL)
	SELECT ID_USUARIO, 3
	FROM FLOPANICMA.USUARIO U
	INNER JOIN FLOPANICMA.PERSONA P
			ON U.ID_USUARIO = P.ID_PERSONA
	INNER JOIN FLOPANICMA.AFILIADO A
			ON P.ID_PERSONA = A.ID_AFILIADO
...
BEGIN
	...
	INSERT INTO FLOPANICMA.USUARIO
	(ID_PERSONA,USERNAME,PASSWORD)
	SELECT ID_PROFESIONAL,'PROF' + CONVERT(VARCHAR(10),ID_PROFESIONAL),FLOPANICMA.PASSWORD_HASH('w23e')
	FROM FLOPANICMA.PERSONA PER
	INNER JOIN FLOPANICMA.PROFESIONAL PROF
			ON PER.ID_PERSONA = PROF.ID_PROFESIONAL;
	
	INSERT INTO FLOPANICMA.USUARIO_ROL
	(ID_USUARIO,ID_ROL)
	SELECT ID_PROFESIONAL,2
	FROM FLOPANICMA.PERSONA PER
	INNER JOIN FLOPANICMA.USUARIO U
...
/* TRIGGERS */
/***********/
CREATE TRIGGER FLOPANICMA.TG_ALTA_AFILIADO ON FLOPANICMA.AFILIADO FOR INSERT
AS
BEGIN
	...
	INSERT INTO FLOPANICMA.USUARIO
	(ID_PERSONA,USERNAME,PASSWORD)
	SELECT ID_AFILIADO,'AFIL'+CONVERT(VARCHAR(10),ID_AFILIADO),FLOPANICMA.PASSWORD_HASH('w23e') FROM INSERTED;

	SELECT @ID_PERSONA = ID_AFILIADO FROM INSERTED;

	SELECT @ID_USUARIO = ID_USUARIO 
	FROM FLOPANICMA.USUARIO AS USU 
...
BEGIN 
	...
	DECLARE @ESTADO_ACTUAL BIT = (SELECT ACTIVO FROM INSERTED);
	DECLARE @ESTADO_ANTERIOR BIT = (SELECT ACTIVO FROM DELETED);

	IF((@ESTADO_ACTUAL<>@ESTADO_ANTERIOR) AND (@ESTADO_ACTUAL=0))
...
CREATE PROCEDURE FLOPANICMA.GET_ROLES_POR_DESCRIPCION
...
BEGIN
	...
	SET @DESCRIPCION_ROL = FLOPANICMA.TRIM(UPPER(@DESCRIPCION_ROL));
...
BEGIN
	...
	SET DESCRIPCION = FLOPANICMA.TRIM(@DESC_ROL)
	WHERE ID_ROL = @ID_ROL;
...
BEGIN
	...
	DECLARE @ID_AFILIADO INT;
	SET @ID_AFILIADO = ( SELECT ID_AFILIADO
						 FROM   FLOPANICMA.AFILIADO
						 WHERE  NRO_AFILIADO = @NRO_AFILIADO );

	DECLARE @FECHA DATETIME;
	SET @FECHA = GETDATE(); -- SE DECIDIO QUE LA FECHA DE COMPRA Y DE IMPRESION ES LA MISMA. ESPECIFICAR EN LA ESTRATEGIA
...
BEGIN TRANSACTION T1
	...
	SET @HORA_INICIO = CONVERT(VARCHAR(2),@INICIO) + ':00'
	SET @HORA_FIN = CONVERT(VARCHAR(2),@FIN) + ':00'	
	...
	IF CONVERT(DATE,@FECHA)=@PERIODO_FIN
		BEGIN
			ROLLBACK TRANSACTION T1;
			SET @FLAG_ERROR = 1
			SET @MENSAJE = 'El dia seleccionado: ' + @DIA + ' no es valido o no hay '+
...
BEGIN
 ...
 SET @ID_PROFESIONAL = ( SELECT TOP 1 ID_PROFESIONAL
       FROM FLOPANICMA.PROFESIONAL
       WHERE MATRICULA = @MATRICULA );
 
 SELECT AG.FECHA 
 FROM   FLOPANICMA.AGENDA AG 
...
BEGIN
 ...
 SET @ID_AFILIADO = (SELECT TOP 1 ID_AFILIADO
					FROM FLOPANICMA.AFILIADO
					WHERE NRO_AFILIADO=@NRO_AFILIADO)
...
/*CANCELACION TURNO AFILIADO*/

CREATE PROCEDURE FLOPANICMA.SP_CANCELACION_TURNO_AFILIADO
@ID_TURNO NUMERIC (18,0),
@DETALLE VARCHAR(255)

AS
BEGIN
	IF @DETALLE='Detalle motivo (Opcional)'
		BEGIN
			INSERT INTO FLOPANICMA.CANCELACION
...
CREATE PROCEDURE FLOPANICMA.SP_CANCELACION_TURNO_PROFESIONAL
@INICIO DATETIME,
@FIN DATETIME,
@ID_PROFESIONAL INT,
@DETALLE VARCHAR(255)
...
BEGIN TRANSACTION --USO TRANSACCION POR LA CANTIDAD DE OPERACIONES QUE INVOLUCRA
	
	--PRIMERO ACTUALIZO LA AGENDA
	
	UPDATE FLOPANICMA.AGENDA
	SET ACTIVO = 0
	WHERE ID_PROFESIONAL = @ID_PROFESIONAL AND
		  FECHA>=@INICIO AND FECHA <= @FIN;

	--SEGUNDO CANCELO LOS TURNOS YA ASIGNADOS DURANTE ESE PERIODO

	IF @DETALLE='Detalle motivo (Opcional)'
		BEGIN
			INSERT INTO FLOPANICMA.CANCELACION
			(ID_TURNO_CANCELADO,COD_TIPO_CANCELACION)
			SELECT ID_TURNO,2 FROM FLOPANICMA.PEDIDO_TURNO
			WHERE ID_PROFESIONAL = @ID_PROFESIONAL AND 
				  FECHA>=@INICIO AND FECHA <= @FIN;
		END
	ELSE
		BEGIN
			INSERT INTO FLOPANICMA.CANCELACION
			(ID_TURNO_CANCELADO,COD_TIPO_CANCELACION,MOTIVO)
			SELECT ID_TURNO,2,@DETALLE FROM FLOPANICMA.PEDIDO_TURNO
			WHERE ID_PROFESIONAL = @ID_PROFESIONAL AND 
				  FECHA>=@INICIO AND FECHA <= @FIN;		
		END
COMMIT
...
BEGIN
	...
	SELECT AFI.NRO_AFILIADO,PER.NOMBRE,PER.APELLIDO,PER.DIRECCION,PER.TELEFONO,PER.E_MAIL,
			AFI.ESTADO_CIVIL,AFI.CANTIDAD_HIJOS,PM.DESCRIPCION
	FROM FLOPANICMA.PERSONA PER LEFT JOIN FLOPANICMA.AFILIADO AFI ON
	PER.ID_PERSONA=AFI.ID_AFILIADO JOIN FLOPANICMA.PLAN_MEDICO AS PM ON AFI.PLAN_MEDICO = PM.ID_PLAN
	WHERE AFI.NRO_AFILIADO = @NRO_AFILIADO;
...
AS
BEGIN
		SELECT TOP 5 COUNT(PED.ID_TURNO) 'Cantidad turnos', 
		(SELECT ESP.DETALLE FROM FLOPANICMA.ESPECIALIDAD ESP WHERE ESP.ID_ESPECIALIDAD = PED.ID_ESPECIALIDAD) 'Especialidad'
		FROM FLOPANICMA.PEDIDO_TURNO PED INNER JOIN FLOPANICMA.CANCELACION CAN ON PED.ID_TURNO = CAN.ID_TURNO_CANCELADO
		WHERE 
		(
		((@SEMESTRE = 1) AND (@MES = 0) AND YEAR(PED.FECHA) = @ANIO AND MONTH(PED.FECHA) < 7) 
		OR 
		((@SEMESTRE = 1) AND (@MES <> 0) AND YEAR(PED.FECHA) = @ANIO AND MONTH(PED.FECHA) = @MES)
		OR 
		((@SEMESTRE = 2) AND (@MES = 0) AND YEAR(PED.FECHA) = @ANIO AND MONTH(PED.FECHA) >= 7) 
		OR
		((@SEMESTRE = 2) AND (@MES <> 0) AND YEAR(PED.FECHA) = @ANIO AND MONTH(PED.FECHA) = @MES)
		)
...
BEGIN TRANSACTION T1
	...
	--PROCEDIMIENTO--
	SET @NRO_BONO = @NRO_AFILIADO;
	SET @NRO_BONO = @NRO_BONO * 10000;
	SET @NRO_BONO = @NRO_BONO + @NRO_CONSULTA;
	
	UPDATE FLOPANICMA.BONO 
	SET BONO_NRO = @NRO_BONO -- Puedo tener hasta 9999 consultas disponibles para un usuario en el sistema. (ESPECIFICAR EN ESTRATEGIA)
	WHERE ID_BONO = @ID_BONO;
	
	INSERT INTO FLOPANICMA.CONSULTA 
	(ID_TURNO,ID_BONO,BONO_NRO,REGISTRO_LLEGADA) 
	VALUES (@ID_TURNO,@ID_BONO,(SELECT BONO_NRO FROM FLOPANICMA.BONO WHERE ID_BONO = @ID_BONO),	@FECHA);
	
	SET @FLAG_ERROR = 0
	SET @MENSAJE = 'Se ha registrado la llegada del afiliado.'
...
BEGIN
		...
		SET @PLAN = (SELECT TOP 1 (ID_PLAN) FROM FLOPANICMA.PLAN_MEDICO WHERE DESCRIPCION = @DETALLE_PLAN)
		
		SELECT TOP 5 COUNT(ped.ID_TURNO) AS 'Cantidad de turnos', 
		(SELECT pl_m.DESCRIPCION FROM FLOPANICMA.PLAN_MEDICO pl_m WHERE pl_m.ID_PLAN=afi.PLAN_MEDICO)'Plan medico afiliado', 
		(SELECT pr.MATRICULA FROM FLOPANICMA.PROFESIONAL pr WHERE pr.ID_PROFESIONAL=ped.ID_PROFESIONAL) 'Matricula profesional',
		(SELECT esp.DETALLE FROM FLOPANICMA.ESPECIALIDAD esp WHERE esp.ID_ESPECIALIDAD=ped.ID_ESPECIALIDAD) 'Especialidad'
		...
		WHERE
		(
		((@SEMESTRE =1) AND (@MES = 0) AND YEAR(ped.FECHA)=@ANIO AND MONTH(ped.FECHA)<7 AND 
		ped.FECHA < GETDATE() AND con.REGISTRO_ATENCION IS NOT NULL AND afi.PLAN_MEDICO = @PLAN) 
		OR 
		((@SEMESTRE =1) AND (@MES <> 0) AND YEAR(ped.FECHA)=@ANIO AND MONTH(ped.FECHA)=@MES AND 
		ped.FECHA < GETDATE() AND con.REGISTRO_ATENCION IS NOT NULL AND afi.PLAN_MEDICO = @PLAN)
		OR 
		((@SEMESTRE =2) AND (@MES = 0) AND YEAR(ped.FECHA)=@ANIO AND MONTH(ped.FECHA)>=7 AND 
		ped.FECHA < GETDATE() AND con.REGISTRO_ATENCION IS NOT NULL AND afi.PLAN_MEDICO = @PLAN) 
		OR
		((@SEMESTRE =2) AND (@MES <> 0) AND YEAR(ped.FECHA)=@ANIO AND MONTH(ped.FECHA)=@MES AND 
		ped.FECHA < GETDATE() AND con.REGISTRO_ATENCION IS NOT NULL AND afi.PLAN_MEDICO = @PLAN)
		)
		GROUP BY afi.PLAN_MEDICO, ped.ID_PROFESIONAL, ped.ID_ESPECIALIDAD
		ORDER BY 'Cantidad de turnos' DESC
END;
GO

CREATE PROCEDURE FLOPANICMA.SP_LEST_PROF_MENOS_HS_TRABAJADAS
@ESPECIALIDAD_DETALLE VARCHAR(255),
@ANIO INT,
@SEMESTRE INT,
@MES INT
AS
BEGIN
		...
		SET @ESPECIALIDAD = (SELECT TOP 1 (ID_ESPECIALIDAD) FROM FLOPANICMA.ESPECIALIDAD WHERE DETALLE = @ESPECIALIDAD_DETALLE) 
	
		SELECT TOP 5 COUNT(DISTINCT ag.FECHA) * 0.5 'Horas trabajadas', 
		(SELECT pr.MATRICULA FROM FLOPANICMA.PROFESIONAL pr WHERE pr.ID_PROFESIONAL=ag.ID_PROFESIONAL) 'Matricula profesional'
		FROM FLOPANICMA.AGENDA ag
		WHERE 
		(
...
BEGIN
	...
	SELECT TOP 5 SUM(cbo.CANTIDAD_BONOS) 'Bonos comprados', afi.NRO_AFILIADO AS 'Nº de afiliado', 
	(CASE WHEN FLOPANICMA.PERTENECE_A_GRUPO(afi.NRO_AFILIADO) = 1 THEN 'No' 
	ELSE 'Si' END) AS 'Pertenece a grupo familiar'
	FROM FLOPANICMA.COMPRA_BONO cbo INNER JOIN FLOPANICMA.AFILIADO afi ON cbo.ID_AFILIADO=afi.ID_AFILIADO
	WHERE (
	((@SEMESTRE =1) AND (@MES = 0) AND YEAR(cbo.FECHA_COMPRA)=@ANIO AND MONTH(cbo.FECHA_COMPRA)<7 AND cbo.FECHA_COMPRA < GETDATE()) 
    	OR 
    	((@SEMESTRE =1) AND (@MES <> 0) AND YEAR(cbo.FECHA_COMPRA)=@ANIO AND MONTH(cbo.FECHA_COMPRA)=@MES AND cbo.FECHA_COMPRA < GETDATE())
	OR 
	((@SEMESTRE =2) AND (@MES = 0) AND YEAR(cbo.FECHA_COMPRA)=@ANIO AND MONTH(cbo.FECHA_COMPRA)>=7 AND cbo.FECHA_COMPRA < GETDATE()) 
	OR
	((@SEMESTRE =2) AND (@MES <> 0) AND YEAR(cbo.FECHA_COMPRA)=@ANIO AND MONTH(cbo.FECHA_COMPRA)=@MES AND cbo.FECHA_COMPRA < GETDATE())
	)
...
CREATE PROCEDURE FLOPANICMA.SP_HISTORIAL_MODIF_PLAN
@NRO_AFILIADO INT,
...
BEGIN
	...
	BEGIN
		SELECT @FLAG_ERROR = 1;
		SELECT @MENSAJE = 'No ingresó ningún nro de afiliado. reintente.';
	END
	ELSE
	BEGIN
		SELECT modif.FECHA, 
		(SELECT pm.DESCRIPCION FROM FLOPANICMA.PLAN_MEDICO pm WHERE pm.ID_PLAN=modif.PLAN_MEDICO_ANTERIOR)'PLAN ANTERIOR',
		modif.DETALLE 
		FROM FLOPANICMA.MODIFICACION as modif
		JOIN FLOPANICMA.AFILIADO as afi ON modif.ID_AFILIADO=afi.ID_AFILIADO
		WHERE modif.DETALLE <> 'BAJA AFILIADO' AND afi.NRO_AFILIADO=@NRO_AFILIADO
	END
END;
GO

CREATE PROCEDURE FLOPANICMA.SP_LEST_ESPECIALIDAD_MAS_BONOS_USADOS
@ANIO INT,
@SEMESTRE INT,
@MES INT
...
BEGIN
	...
	SELECT TOP 5 COUNT(con.ID_BONO) 'Cantidad bonos', 
	(SELECT esp.DETALLE FROM FLOPANICMA.ESPECIALIDAD esp WHERE esp.ID_ESPECIALIDAD=ped_t.ID_ESPECIALIDAD) 'Especialidad'
	FROM FLOPANICMA.CONSULTA con LEFT JOIN FLOPANICMA.PEDIDO_TURNO ped_t
	ON con.ID_TURNO=ped_t.ID_TURNO
	WHERE (
	((@SEMESTRE =1) AND (@MES = 0) AND YEAR(con.REGISTRO_LLEGADA)=@ANIO AND MONTH(con.REGISTRO_LLEGADA)<7 AND con.REGISTRO_LLEGADA < GETDATE()) 
    	OR 
    	((@SEMESTRE =1) AND (@MES <> 0) AND YEAR(con.REGISTRO_LLEGADA)=@ANIO AND MONTH(con.REGISTRO_LLEGADA)=@MES AND con.REGISTRO_LLEGADA < GETDATE())
	OR 
	((@SEMESTRE =2) AND (@MES = 0) AND YEAR(con.REGISTRO_LLEGADA)=@ANIO AND MONTH(con.REGISTRO_LLEGADA)>=7 AND con.REGISTRO_LLEGADA < GETDATE()) 
	OR
	((@SEMESTRE =2) AND (@MES <> 0) AND YEAR(con.REGISTRO_LLEGADA)=@ANIO AND MONTH(con.REGISTRO_LLEGADA)=@MES AND con.REGISTRO_LLEGADA < GETDATE())
	)
	GROUP BY ped_t.ID_ESPECIALIDAD
	ORDER BY 'Cantidad bonos' DESC;
	
END;
GO

CREATE PROCEDURE FLOPANICMA.SP_ALTA_AFILIADO_RAIZ
@NOMBRE VARCHAR(255),
@APELLIDO VARCHAR(255),
@DIRECCION VARCHAR (255),
@TELEFONO NUMERIC(18,0),
...
INSERT INTO FLOPANICMA.PERSONA
(ID_PERSONA,NOMBRE,APELLIDO,DIRECCION,TELEFONO,E_MAIL,F_NACIMIENTO,TIPO_DOCUMENTO,NRO_DOCUMENTO,SEXO)
VALUES
(NEXT VALUE FOR FLOPANICMA.PERSONA_SEQ,@NOMBRE,@APELLIDO,@DIRECCION,@TELEFONO,@E_MAIL,
@F_NACIMIENTO,@TIPO_DOCUMENTO,@NRO_DOCUMENTO,@SEXO);

SET @ID_AFILIADO = CONVERT (INT,(SELECT CURRENT_VALUE FROM SYS.SEQUENCES WHERE [NAME]='PERSONA_SEQ'));

INSERT INTO FLOPANICMA.AFILIADO
(ID_AFILIADO,NRO_AFILIADO,PLAN_MEDICO,ESTADO_CIVIL,CANTIDAD_HIJOS,NRO_CONSULTA)
VALUES
(@ID_AFILIADO,NEXT VALUE FOR FLOPANICMA.AFILIADO_SEQ,@PLAN_MEDICO,@ESTADO_CIVIL,@CANTIDAD_FAMILIARES,0);

SET @NRO_AFILIADO = CONVERT (INT,(SELECT CURRENT_VALUE FROM SYS.SEQUENCES WHERE [NAME]='AFILIADO_SEQ'));
COMMIT
GO

CREATE PROCEDURE FLOPANICMA.SP_FAMILIARES
@NRO_AFILIADO INT
...
SET @BASE = @NRO_AFILIADO/100

SELECT NRO_AFILIADO,APELLIDO,NOMBRE 
FROM FLOPANICMA.AFILIADO AS AFI JOIN FLOPANICMA.PERSONA AS PER
	 ON AFI.ID_AFILIADO=PER.ID_PERSONA
WHERE NRO_AFILIADO <> @NRO_AFILIADO
	  AND NRO_AFILIADO BETWEEN (@BASE*100) AND ((@BASE+1)*100)
ORDER BY NRO_AFILIADO DESC
	 
END;
GO

CREATE PROCEDURE FLOPANICMA.SP_MODIFICAR_AFILIADO

@DIRECCION VARCHAR(255),
@TELEFONO NUMERIC(18,0),
@E_MAIL VARCHAR(255),
@DESC_PLAN_MEDICO  VARCHAR(255),
@ESTADO_CIVIL VARCHAR(50),
@CANTIDAD_HIJOS INT,
@NRO_AFILIADO INT,
@FECHA DATETIME,
...
SET @ID_PERSONA = (SELECT ID_PERSONA FROM FLOPANICMA.PERSONA AS PER JOIN 
						  FLOPANICMA.AFILIADO AS AFI ON PER.ID_PERSONA=AFI.ID_AFILIADO
					WHERE AFI.NRO_AFILIADO = @NRO_AFILIADO)

SET @PLAN_MEDICO = (SELECT top 1 (ID_PLAN) FROM FLOPANICMA.PLAN_MEDICO WHERE DESCRIPCION = @DESC_PLAN_MEDICO);

SET @PLAN_ANTERIOR = (SELECT top 1 (PLAN_MEDICO) FROM FLOPANICMA.AFILIADO WHERE NRO_AFILIADO=@NRO_AFILIADO)
...
UPDATE FLOPANICMA.AFILIADO
SET ESTADO_CIVIL=@ESTADO_CIVIL,CANTIDAD_HIJOS=@CANTIDAD_HIJOS,PLAN_MEDICO=@PLAN_MEDICO
WHERE NRO_AFILIADO=@NRO_AFILIADO

IF (@PLAN_MEDICO<>@PLAN_ANTERIOR)
BEGIN
	INSERT INTO FLOPANICMA.MODIFICACION
	(ID_AFILIADO,FECHA,DETALLE,PLAN_MEDICO_ANTERIOR)
	VALUES
	(@ID_PERSONA,@FECHA,@DETALLE,@PLAN_ANTERIOR);
END;
...
CREATE PROCEDURE FLOPANICMA.SP_ALTA_AFILIADO_ASOCIADO
...
(ID_AFILIADO,NRO_AFILIADO,PLAN_MEDICO,ESTADO_CIVIL,CANTIDAD_HIJOS,NRO_CONSULTA)
VALUES
(@ID_AFILIADO,@NRO_AFILIADO,@PLAN_MEDICO,@ESTADO_CIVIL,@CANTIDAD_FAMILIARES,0);

COMMIT

GO

CREATE PROCEDURE FLOPANICMA.SP_VINCULAR_AFILIADOS 

@NUEVO_NUMERO_AFILIADO INT,
@ID_AFILIADO_A_VINCULAR INT

AS
BEGIN
	
	UPDATE FLOPANICMA.AFILIADO
	SET NRO_AFILIADO = @NUEVO_NUMERO_AFILIADO
	WHERE ID_AFILIADO=@ID_AFILIADO_A_VINCULAR;
END;
