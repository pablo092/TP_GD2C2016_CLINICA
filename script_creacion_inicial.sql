...
BEGIN
	...
	BEGIN
		...
		WHERE YEAR(PED.FECHA)=@ANIO AND (MONTH(PED.FECHA)<7 OR MONTH(PED.FECHA)=@MES)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			WHERE afi.PLAN_MEDICO = @PLAN AND YEAR(ped.FECHA)=@ANIO AND (MONTH(ped.FECHA)<7 OR MONTH(ped.FECHA)=@MES)
		...
		BEGIN
			...
			BEGIN
				...
				FROM FLOPANICMA.CONSULTA con LEFT JOIN FLOPANICMA.PEDIDO_TURNO ped ON ped.ID_TURNO=con.ID_TURNO 
				INNER JOIN FLOPANICMA.AFILIADO afi ON ped.ID_AFILIADO=afi.ID_AFILIADO
				WHERE afi.PLAN_MEDICO = @PLAN AND YEAR(ped.FECHA)=@ANIO
				GROUP BY afi.PLAN_MEDICO, ped.ID_PROFESIONAL, ped.ID_ESPECIALIDAD
				ORDER BY cantidad_turno DESC
			END
		END
