...
BEGIN
	...
	BEGIN
		...
		PER.SEXO, AFI.ESTADO_CIVIL, AFI.CANTIDAD_HIJOS, AFI.NRO_AFILIADO
...
AS
BEGIN
		SELECT TOP 5 COUNT(DISTINCT PED.ID_TURNO) 'Cantidad turnos', 
		...
		WHERE 
		(
		((@SEMESTRE =1) AND (@MES = 0) AND YEAR(PED.FECHA)=@ANIO AND MONTH(PED.FECHA)<7 AND PED.FECHA < GETDATE()) 
		OR 
		((@SEMESTRE =1) AND (@MES <> 0) AND YEAR(PED.FECHA)=@ANIO AND MONTH(PED.FECHA)=@MES AND PED.FECHA < GETDATE())
		OR 
		((@SEMESTRE =2) AND (@MES = 0) AND YEAR(PED.FECHA)=@ANIO AND MONTH(PED.FECHA)>=7 AND PED.FECHA < GETDATE()) 
		OR
		((@SEMESTRE =2) AND (@MES <> 0) AND YEAR(PED.FECHA)=@ANIO AND MONTH(PED.FECHA)=@MES AND PED.FECHA < GETDATE())
		)
...
AS
BEGIN
		SELECT TOP 5 COUNT(ped.ID_TURNO) 'Cantidad_turnos', 
		(SELECT pl_m.DESCRIPCION FROM FLOPANICMA.PLAN_MEDICO pl_m WHERE pl_m.ID_PLAN=afi.PLAN_MEDICO)'Plan medico', 
...
BEGIN
	...
	SELECT TOP 5 SUM(cbo.CANTIDAD_BONOS) 'Bonos_comprados', afi.NRO_AFILIADO, 
	(CASE WHEN RIGHT(CONVERT(varchar(20), afi.NRO_AFILIADO), 2)='01' THEN 'afiliado raíz' 
