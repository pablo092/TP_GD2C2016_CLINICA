...
BEGIN
	...
	(ID_BONO,ID_TURNO,BONO_NRO,SINTOMA,DIAGNOSTICO,REGISTRO_LLEGADA)
	SELECT (SELECT ID_BONO FROM FLOPANICMA.BONO BONOTAB WHERE MIGCON.BONO_NUMERO = BONOTAB.BONO_NRO), NRO_TURNO,BONO_NUMERO,SINTOMA,ENFERMEDAD,FECHA_TURNO 
	FROM @MIGRA_CONSULTAS AS MIGCON
...
BEGIN
	...
	SELECT @FECHA_TURNO = FECHA FROM PEDIDO_TURNO WHERE ID_TURNO = @ID_TURNO;
		...
		/* HACER LA CANCELACION DE PARTE DEL AFILIADO */
			...
			SELECT PED.ID_TURNO, 1, @MOTIVO FROM FLOPANICMA.PEDIDO_TURNO AS PED INNER JOIN
	...
	/* CASO EN EL QUE ES UN PROFESIONAL*/
		...
		/* VERIFICA QUE LA FECHA UNICA SEA VALIDA Y HACE LA CANCELACION DE TURNOS */
			...
			INSERT INTO FLOPANICMA.CANCELACION
			(ID_TURNO_CANCELADO,COD_TIPO_CANCELACION,MOTIVO)
				SELECT PED.ID_TURNO, 3, @MOTIVO FROM FLOPANICMA.PEDIDO_TURNO AS PED INNER JOIN 
				FLOPANICMA.CONSULTA CON ON	PED.ID_TURNO = CON.ID_TURNO
		...
		/* SI LA FECHA UNICA DE PROF ES VACIA, SE ENTIENDE QUE PASÓ FECHA DESDE-HASTA */
			...
			INSERT INTO FLOPANICMA.CANCELACION
			(ID_TURNO_CANCELADO,COD_TIPO_CANCELACION,MOTIVO)
				SELECT PED.ID_TURNO, 2, @MOTIVO FROM FLOPANICMA.PEDIDO_TURNO AS PED INNER JOIN 
				FLOPANICMA.CONSULTA CON ON	PED.ID_TURNO = CON.ID_TURNO
...
BEGIN
	...
	BEGIN
		...
		WHERE YEAR(PED.FECHA)=@ANIO OR MONTH(PED.FECHA)<7 OR MONTH(PED.FECHA)=@MES
...
BEGIN
	...
	SET SINTOMA = @SINTOMA, DIAGNOSTICO = @DIAGNOSTICO, REGISTRO_ATENCION = GETDATE()
