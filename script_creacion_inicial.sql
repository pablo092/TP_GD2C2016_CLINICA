...
BEGIN
	...
	SET @PASSWORD = FLOPANICMA.TRIM(UPPER(@PASSWORD));
...
CREATE PROCEDURE FLOPANICMA.SP_AFILIADO_RAIZ_ALTA
@NOMBRE VARCHAR(255),
@APELLIDO VARCHAR(255),
...
AS
BEGIN
	/* info que se autocompleta con default si no la ingresan */
	IF(@E_MAIL IS NULL)
	BEGIN
	SET @E_MAIL = 'SIN E-MAIL';
	END
	IF(@TIPO_DOCUMENTO IS NULL)
	BEGIN
	SET @E_MAIL = 'DNI';
	END
	/* info que de no ingresarse provoca un error */
	IF(@NOMBRE IS NULL OR @APELLIDO IS NULL OR @DIRECCION IS NULL OR @TELEFONO IS NULL OR @F_NACIMIENTO IS NULL OR
	@NRO_DOCUMENTO IS NULL OR @SEXO IS NULL)
	BEGIN
		SET @FLAG_ERROR =1;
		SET @MENSAJE = 
		'Información incompleta en: Nombre, Apellido, Direccion, telefono, fecha de nacimiento, nro de documento, sexo, estado civil o plan medico';
	END
	ELSE
	BEGIN
		SELECT @ID_PERSONA= NEXT VALUE FOR FLOPANICMA.PERSONA_SEQ;

		/* Alta en PERSONA */
		INSERT INTO FLOPANICMA.PERSONA
		VALUES(@ID_PERSONA,	@NOMBRE, @APELLIDO, @DIRECCION, @TELEFONO, @E_MAIL, @F_NACIMIENTO, @TIPO_DOCUMENTO, @NRO_DOCUMENTO, @SEXO);

		/* Alta tambien en AFILIADO */
		INSERT INTO FLOPANICMA.AFILIADO
		(ID_AFILIADO, NRO_AFILIADO, PLAN_MEDICO, ACTIVO, ESTADO_CIVIL, CANTIDAD_HIJOS, NRO_CONSULTA)
		VALUES(@ID_PERSONA,((NEXT VALUE FOR FLOPANICMA.AFILIADO_SEQ) + 1),@PLAN_MEDICO,1,@ESTADO_CIVIL,@CANTIDAD_HIJOS,0);
