...
(		
		...
		CONSTRAINT PK_AGENDA PRIMARY KEY (ID_PROFESIONAL,DIA,HORA_TURNO,ID_ESPECIALIDAD,PERIODO_INICIO,PERIODO_FIN),
...
CREATE FUNCTION FLOPANICMA.HORARIO_ASIGNADO(@ID_PROFESIONAL INT,@HORA_INICIO INT,@HORA_FIN INT,@DIA VARCHAR(10),@PERIODO_INICIO DATE)
	RETURNS BIT
	 
	BEGIN 
	
		DECLARE @CANTIDAD_ASIGNADA INT = 0;
		
		SET @CANTIDAD_ASIGNADA = (SELECT COUNT (*) FROM FLOPANICMA.AGENDA
					 WHERE ID_PROFESIONAL = @ID_PROFESIONAL AND 
					 PERIODO_FIN >= @PERIODO_INICIO AND DATENAME(DW,DIA) = @DIA
					 AND HORA_TURNO BETWEEN @HORA_INICIO AND @HORA_FIN)
...
CREATE FUNCTION FLOPANICMA.HORAS_ACUMULADAS(@ID_PROFESIONAL INT,@TURNOS_AGREGADOS INT,@INICIO DATE,@FIN DATE)
	RETURNS BIT
	
	BEGIN
		DECLARE @HORAS_ACUMULADAS INT;
		
		SET @HORAS_ACUMULADAS = (SELECT SUM(0.5) FROM FLOPANICMA.AGENDA 
					 WHERE ID_PROFESIONAL = @ID_PROFESIONAL AND 
					 PERIODO_FIN >= @INICIO)
...
BEGIN
	...
	SELECT DISTINCT PROTAB.ID_PROFESIONAL,FECHA,CONVERT(TIME(0),FECHA),ESPTAB.ID_ESPECIALIDAD,'2015-01-01','2015-12-30'
...
/*
Quita una funcionalidad de un rol existente
IN: @USERNAME varchar(255), @Desc_rol varchar(50)
OUT: @FLAG_ERROR INT, @MENSAJE VARCHAR(255)
*/

CREATE PROCEDURE FLOPANICMA.SP_ABM_ROL_ELIMINAR_IN_USER
@USERNAME VARCHAR(255),
@DESC_ROL VARCHAR(50),
...
AS
BEGIN	
	DECLARE @ID_ROL_NUEVO INT = 0;
	DECLARE @ID_USUARIO_NUEVO INT = 0;
	DECLARE @CANTIDAD INT = 0;

	SELECT @DESC_ROL = FLOPANICMA.TRIM(UPPER(@DESC_ROL));
	SELECT @USERNAME = FLOPANICMA.TRIM(UPPER(@USERNAME));

	SELECT @FLAG_ERROR = 0;
	SELECT @MENSAJE = '';

	SELECT @ID_ROL_NUEVO = ID_ROL 
	FROM FLOPANICMA.ROL R
	WHERE R.DESCRIPCION = @DESC_ROL;

	IF(@ID_ROL_NUEVO IS NULL OR @ID_ROL_NUEVO = 0)
	BEGIN 
		SELECT @FLAG_ERROR = 1;
		SELECT @MENSAJE = 'El Rol especificado no existe.';
		RETURN;
	END

	SELECT @ID_USUARIO_NUEVO = ID_USUARIO
	FROM FLOPANICMA.USUARIO F
	WHERE F.USERNAME = @USERNAME;

	IF(@ID_USUARIO_NUEVO IS NULL OR @ID_USUARIO_NUEVO = 0)
	BEGIN 
		SELECT @FLAG_ERROR = 1;
		SELECT @MENSAJE = 'El Usuario especificado no existe.';
		RETURN;
	END

	SELECT @CANTIDAD = count(*)
	FROM FLOPANICMA.USUARIO_ROL 
	WHERE ID_ROL = @ID_ROL_NUEVO 
	AND ID_USUARIO = @ID_USUARIO_NUEVO;

	IF(@CANTIDAD = 0)
	BEGIN 
		SELECT @FLAG_ERROR = 1;
		SELECT @MENSAJE = 'El Rol no se encuentra asignado al Usuario especificado.';
		RETURN;
	END

	DELETE FROM FLOPANICMA.USUARIO_ROL
	WHERE ID_ROL = @ID_ROL_NUEVO
	AND ID_USUARIO = @ID_USUARIO_NUEVO;
...
CREATE PROCEDURE FLOPANICMA.SP_REGISTRAR_AGENDA -- ME FALTA TERMINARLO MAS TARDE SI LLEGO LO SUBO DE VUELTA
...
BEGIN TRANSACTION
	...
	IF (@DIA = 'SÁBADO') AND ((@HORA_INICIO < 10) OR (@HORA_FIN >15))
		BEGIN
			SET @FLAG_ERROR = 1
	...
	IF (@HORA_INICIO < 7) OR (@HORA_FIN > 20)
		BEGIN
			SET @FLAG_ERROR = 1
			SET @MENSAJE = 'EL HORARIO ELEGIDO ESTA EN UN RANGO NO VALIDO'
			RETURN;
	...
	IF FLOPANICMA.HORARIO_ASIGNADO(@ID_PROFESIONAL,@HORA_INICIO,@HORA_FIN,@DIA,@PERIODO_INICIO,@PERIODO_FIN) = 1
		BEGIN
			SET @FLAG_ERROR = 1
			SET @MENSAJE = 'EL HORARIO ELEGIDO YA TIENE ACTIVIDAD REGISTRADA'
			RETURN;
		END;
	
	IF FLOPANICMA.HORAS_ACUMULADAS(@ID_PROFESIONAL,@TURNOS_AGREGADOS) = 1
		BEGIN
			SET @FLAG_ERROR = 1
			SET @MENSAJE = 'LOS HORARIOS INGRESADOS SUPERAN EL MAXIMO PERMITIDO POR SEMANA'
			RETURN;
		END;
	...
	BEGIN
		INSERT INTO FLOPANICMA.AGENDA
		(ID_PROFESIONAL,DIA,HORA_TURNO,ID_ESPECIALIDAD,PERIODO_INICIO,PERIODO_FIN)
		VALUES(@ID_PROFESIONAL,@DIA,DATEADD(MINUTE,30*(@TURNOS_AGREGADOS-1),@HORA_INICIO),@ID_ESPECIALIDAD,
